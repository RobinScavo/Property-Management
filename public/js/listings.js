...
filterButton.addEventListener('click', () => {
    ...
    if (!bedroomFilter.value && !bathroomFilter.value && !rentFilter.value) return;

    listings.forEach((listing) => {
        const numberOfBedrooms = +listing.childNodes[3].childNodes[1].innerHTML.trim()[0];
        const numberOfBath = +listing.childNodes[3].childNodes[3].innerHTML.trim()[0];
        const rent = +listing.childNodes[5].childNodes[1].innerHTML.trim().split('/')[0].slice(1);
